...
#define RTL8367_SMI_CLK_DELAY               10 /* nsec */

void rtl8367_smi_powersave_mode(bool enable)
{
}
...
{
#if 0
	if (smi->powersave_capable && smi->low_speed_clock)
		udelay(300);
	else
#endif
		ndelay(RTL8367_SMI_CLK_DELAY);
}
...
static int rtl8367_smi_read_byte0(struct rtl8367_smi *smi, u8 *data)
{
	u32 t;
...
static int rtl8367_smi_read_byte1(struct rtl8367_smi *smi, u8 *data)
{
	u32 t;
...
int rtl8367_smi_read_reg(struct rtl8367_smi *smi, u32 addr, u32 *data)
{
	...
	/* send READ command */
	ret = rtl8367_smi_write_byte(smi, 0x0b << 4 | 0x04 << 1 | 0x01);
	if (ret)
		goto out;
...
EXPORT_SYMBOL_GPL(rtl8367_smi_read_reg);
...
int rtl8367_smi_write_reg(struct rtl8367_smi *smi, u32 addr, u32 data)
{
	...
	/* send WRITE command */
	ret = rtl8367_smi_write_byte(smi, 0x0b << 4 | 0x04 << 1 | 0x00);
	if (ret)
		goto out;
...
EXPORT_SYMBOL_GPL(rtl8367_smi_init);
...
EXPORT_SYMBOL_GPL(rtl8367_smi_cleanup);

MODULE_DESCRIPTION("Realtek RTL8367 SMI interface driver");
