...
static void gpio_button_timeout(unsigned long p)
{
	struct input_polled_dev *dev = ((struct gpio_button_data *)p)->dev;
	struct gpio_buttons_dev *bdev = dev->private;

	struct gpio_buttons_platform_data *pdata = bdev->pdata;
	struct input_dev *input = dev->input;

	int i = ((struct gpio_button_data *)p - bdev->data);
	struct gpio_button *button = &pdata->buttons[i];
	unsigned int type = button->type ?: EV_KEY;

	dev_info(bdev->dev, "%s %s\n", button->desc, "held");
				      ...
				      struct gpio_button_data *bdata)
{
	...
	if (state != bdata->last_state) {
		struct input_polled_dev *dev = bdata->dev;
		struct gpio_buttons_dev *bdev = dev->private;

		unsigned int type = button->type ?: EV_KEY;
		bool pushed = (!(state ^ button->active_low));

		dev_info(bdev->dev, "%s %s\n", button->desc, pushed ? "pushed" : "released");
		...
		if (pushed) {
			...
			mod_timer(&bdata->timeout, jiffies + (4 * HZ));
		} else {
			del_timer(&bdata->timeout);
		}
	}
}
...
static int __devinit gpio_buttons_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		bdev->data[i].dev = poll_dev;
		setup_timer(&bdev->data[i].timeout, gpio_button_timeout, (unsigned long)&bdev->data[i]);
