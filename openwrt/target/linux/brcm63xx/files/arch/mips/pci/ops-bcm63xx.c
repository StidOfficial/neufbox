...
u32 pci63xx_wlan_soft_config_space[WLAN_ONCHIP_DEV_NUM][WLAN_ONCHIP_PCI_HDR_DW_LEN] = {
				...
				u32 *val)
{
	...
	iob();
	data = __raw_readl(pci_iospace_start);
		...
		if ((where >= PCI_BASE_ADDRESS_0) && (where <= PCI_BASE_ADDRESS_5)) {
			if (pci_mem_size_rd) {
				...
				if ((*val & PCI_BASE_ADDRESS_SPACE) != PCI_BASE_ADDRESS_SPACE_IO)
					*val &= (0xFFFF0000 + ~PCI_BASE_ADDRESS_MEM_MASK);
			}
		}
		break;
	}
				 ...
				 u32 val)
{
	...
	__raw_writel(data, pci_iospace_start);
	wmb();
