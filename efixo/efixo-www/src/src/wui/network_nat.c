...
DISPLAY_HANDLER
{
        ...
        if(nbd_user_config_get("nat_alg_sip", buffer, sizeof(buffer)) == NBD_SUCCESS)
        {
                ewf_hdf_set("sipalg_active", buffer);
        }
#endif
...
FORM_HANDLER
{
        ...
        else if (action != NULL && nbu_string_compare(action, "sipalg_submit") == NBU_SUCCESS)
        {
                restart = 0;

                if(ewf_hdf_sget_copy("sipalg_active", buffer, sizeof(buffer)) == 0)
                {
                        if(nbd_user_config_set("nat_alg_sip", buffer) == NBD_SUCCESS)
                        {
                                nbu_log_debug("set nat_alg_sip %s", buffer);
                                restart++;
                        }
                }
